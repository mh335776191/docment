...
namespace Plugin.Framework
{
    public class Manage:IManage
    {
        ...
        {
            //注册插件控制器工厂。
            ControllerBuilder.Current.SetControllerFactory(new MVC.PluginControllerFactory());
            //注册插件模板引擎。
            ViewEngines.Engines.Clear();
            ViewEngines.Engines.Add(new MVC.PluginRazorViewEngine());

            var plugins=fileParser.Load();
            List<MVCRouteConfig> routes = new List<MVCRouteConfig>();
            ...
            foreach (var plugin in plugins)
            {
                ...
                if (plugin.Status == PluginStatus.Usable && plugin.Type == PluginType.Module)
                {
                    ...
                    var routeConfigType = typeof(MVCRouteConfig);
                    foreach (var t in plugin.Types)
                    {
                        ...
                        else if (t.BaseType.Name == routeConfigType.Name)
                        {
                            MVCRouteConfig route = (MVCRouteConfig)Activator.CreateInstance(t);
                            routes.Add(route);
                        }
                        else if (t != null &&
                                  t.IsPublic &&
