...
namespace Plugin.Framework
{
    public class ZipManage : IZipManage
    {
        ...
        {
            System.IO.File.Delete(Path.Combine(zipsPath, zipName));
            PluginZipInfo zipInfo;
            if (zipInfoList.TryRemove(zipName, out zipInfo))
            {
                System.IO.File.Delete(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Plugins", "zips", zipInfo.ZipName));
                SaveInfos();
            }
        }
