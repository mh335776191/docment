...
//点赞动画
function UpDataAnimate(aobj,offx,offy) {
    idrandom = new Date().getTime();
    var left = parseInt($(aobj).offset().left) + offx, top = parseInt($(aobj).offset().top) - offy, obj = $(aobj);
    $(aobj).append('<div id="zhan' + idrandom + '"><b>+1<\/b></\div>');
    $('#zhan' + idrandom).css({ 'position': 'absolute', 'z-index': '1', 'color': '#C30', 'left': left + 'px', 'top': top + 'px' }).animate({ top: top - 10, left: left + 10 }, 'slow', function () {
